...
use amfi::agent::{AgentTraceStep, AgentTrajectory, InformationSet, Policy, PresentPossibleActions, ScoringInformationSet};
...
<DP as DomainParameters>::ActionType: ConvertToTensor<A2T> {
    ...
    fn train_on_trajectories<R: Fn(&AgentTraceStep<DP, <Self as Policy<DP>>::InfoSetType>) -> Tensor>(&mut self, trajectories: &[AgentTrajectory<DP, <Self as Policy<DP>>::InfoSetType>], reward_f: R) -> Result<(), AmfiRLError<DP>> {
        ...
        for t in trajectories{
            ...
            trace!("Payoff tensor: {:?}", discounted_rewards_tensor_vec);
